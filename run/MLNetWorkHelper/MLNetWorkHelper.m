...
+ (NSURLSessionTask *)GET:(NSString *)URL
    ...
    NSURLSessionTask *sessionTask = [_sessionManager GET:URL parameters:parameters progress:^(NSProgress * _Nonnull downloadProgress) {
        ...
        if (_isOpenLog) {
//            MLLog(@"header = %@",task.currentRequest.allHTTPHeaderFields);
        }
        [[self allSessionTask] removeObject:task];
...
+ (NSURLSessionTask *)POST:(NSString *)URL
    ...
    if (_isOpenLog) {MLLog(@"url = %@,param = %@",URL,parameters);}
...
+ (NSURLSessionTask *)uploadFileWithURL:(NSString *)URL
    ...
    NSURLSessionTask *sessionTask = [_sessionManager POST:URL parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        NSError *error = nil;
          MLLog(@"file %@ %@",filePath,@([[NSFileManager defaultManager] fileExistsAtPath:filePath]));
...
//    {
//        MLLog(@"userInfo %@",[XQLoginExample lastUserInfo]);
//        //  如果登录了就传登录信息
