...
@interface xqCurrentDateModel : NSObject
...
@implementation xqCurrentDateModel

@end

#import "xq_PickerView.h"
#import "NSDate+Extend.h"
#import "NSArray+zh_SafeAccess.h"
#import "zhPopupController.h""

typedef NS_ENUM(NSInteger, xqDateType) { // 应该与allDataArray数组顺序一致
    xqDateTypeYear = 0,
    xqDateTypeMonth,
    xqDateTypeDay,
    xqDateTypeHours,
...
@interface xq_PickerView () <UIPickerViewDataSource, UIPickerViewDelegate>
...
@property (nonatomic, strong) xqCurrentDateModel *currentDateModel; // 记录当前选中的数据

@end

@implementation xq_PickerView
...
- (void)initialSelectedItem {
    ...
    xqCurrentDateModel *model = self.currentDateModel;
...
- (void)layoutSubviews {
    ...
    _titleLabel.top = paddingTop;
    
    _pickerView.size = CGSizeMake(self.width, 216);
    _pickerView.top = _titleLabel.bottom + 10;
}
...
- (void)showTitleWithModel:(xqCurrentDateModel *)model {
...
// 当前时间数据模型
- (xqCurrentDateModel *)currentDateModel {
    if (!_currentDateModel) {
        _currentDateModel = [xqCurrentDateModel new];
    }
...
@implementation xq_PickerView (show)

#pragma mark - Picker style

+ (void)showCallBackBlock:(void (^) (NSString *str))block{
    CGRect rect = CGRectMake(0, 0, KWIDTH, 275);
   xq_PickerView *pView = [[xq_PickerView alloc] initWithFrame:rect];

    UIViewController *vc = [UIApplication sharedApplication].keyWindow.rootViewController;
    pView.saveClickedBlock = ^(xq_PickerView *pickerView) {
        NSString *message = [NSString stringWithFormat:@"%@", pickerView.selectedTimeString];
        vc.zh_popupController.didDismiss = ^(zhPopupController * _Nonnull popupController) {
            block(message);
        };
