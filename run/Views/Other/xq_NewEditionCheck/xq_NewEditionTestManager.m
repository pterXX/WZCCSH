...
#import "xq_AppStoreInfoModel.h"

@interface xq_NewEditionTestManager ()
//本地info文件
@property (strong,nonatomic)NSDictionary *infoDict;
@end

@implementation xq_NewEditionTestManager
...
+ (instancetype)shareManager {
    static xq_NewEditionTestManager *instance;
...
+(void)checkNewEditionWithAppID:(NSString *)appID CustomAlert:(CheckVersionBlock)checkVersionBlock {
    [[self shareManager] getAppStoreVersion:appID sucess:^(xq_AppStoreInfoModel *model) {
        if(checkVersionBlock)checkVersionBlock(model);
    }];
}

- (void)checkNewVersion:(NSString *)appID ctrl:(UIViewController *)containCtrl {
    [self getAppStoreVersion:appID sucess:^(xq_AppStoreInfoModel *model) {
...
#pragma mark - 立即升级
- (void)updateRightNow:(xq_AppStoreInfoModel *)model {
...
- (void)getAppStoreVersion:(NSString *)appID sucess:(void(^)(xq_AppStoreInfoModel *))update {
    ...
    [self getAppStoreInfo:appID success:^(NSDictionary *respDict) {
        ...
        if (resultCount == 1) {
            ...
            //字典转模型
            xq_AppStoreInfoModel *model = [[xq_AppStoreInfoModel alloc] init];
